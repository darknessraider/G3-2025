...
public class DriveCommands {
            ...
            if (elevatorClimb) {
                ...
                return Commands.sequence(
                        new DriveToPoint(drive,
                                () -> new Pose2d(
                                        driveTranslation2d
                                                .nearest(List.of(Barge.closeCage, Barge.middleCage, Barge.farCage)
                                                        .stream()
                                                        .map(AllianceFlipUtil::apply).toList()),
                                        AllianceFlipUtil.apply(Rotation2d.kZero))
                                        .transformBy(
                                                new Transform2d(Inches.of(6), Inches.zero(), Rotation2d.k180deg))));
            }
                    ...
                    return Commands.sequence(
                            new DriveToPoint(drive,
                                    () -> CoralStation.leftCenterFace
                                            .rotateAround(CoralStation.leftCenterFace.getTranslation(),
                                                    Rotation2d.kCCW_90deg)
                    ...
                    return Commands.sequence(
                            new DriveToPoint(drive,
                                    () -> CoralStation.rightCenterFace
                                            .rotateAround(CoralStation.rightCenterFace.getTranslation(),
                                                    Rotation2d.kCCW_90deg)
                                            .transformBy(kStationOffset)),
                            drive.stopCommand());
                }
    ...
    public static Command alignToGamePiece(Drive drive, Supplier<Double> interpolationFactorSupplier,
        ...
        Supplier<Translation2d> getTranslation = () -> {
            ...
            poseError = new Translation2d(
                    MathUtil.clamp(poseError.getNorm(), 0, kMaxLinearVelocity.get().in(MetersPerSecond)),
                    poseError.getAngle());
    ...
    public static Command alignToAlgaeReef(Drive drive, LED led, Supplier<ReefFace> reefFaceSupplier) {
        Supplier<Pose2d> branchPoseSupplier = () -> {
            ...
            Pose2d target = FieldConstants.Reef.branchPositions2d.get(face.ordinal() * 2 + 1)
